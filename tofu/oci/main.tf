  ...
  # ------------------------------------------------------------------
  # Segunda instância OCI — mesmo compartment, subnet e SG da primeira
  # Descomentar e rodar: tofu apply -auto-approve
  # Depois: ./convert-to-nixos.sh <IP>
  # ------------------------------------------------------------------
  resource "oci_core_instance" "ocinix2" {
   compartment_id      = oci_identity_compartment.ocinix_compartment.id
   availability_domain = var.availability_domain
   display_name        = "ocinix2"
   shape               = "VM.Standard.E3.Flex"
