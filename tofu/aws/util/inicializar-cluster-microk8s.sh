...
do_join() {
  ...
  NODE_TOKEN=$(echo "$add_node_out" | grep "${MASTER_CONTAINER_VPN_IP}:25000" | grep -oP '(?<=join )\S+:25000/\S+' | head -1 | grep -oP '[^/]+$')
  ...
  local JOIN_CMD="sudo microk8s join ${MASTER_CONTAINER_VPN_IP}:25000/${NODE_TOKEN}"
