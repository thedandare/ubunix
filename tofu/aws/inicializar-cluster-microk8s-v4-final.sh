...
# ==============================================================================
# inicializar-cluster-microk8s-v4-final.sh
#
...
#   - HTTPS autoassinado com SAN para todos os IPs públicos selecionados
#   - Rota /serverpod -> Service Serverpod porta 8080, removendo o prefixo
#   - Rota /control -> Service Serverpod porta 8082, removendo o prefixo
#   - Senha do postgres reconciliada declarativamente pelo CloudNativePG
#   - Proxy Incus TCP 443 declarado no template Nix de cada host
...
# Todos os hosts selecionados publicam TCP/443 pelo template Nix.
# O certificado TLS inclui todos esses IPs como SAN.
SERVERPOD_PUBLIC_IPS=("$MASTER_IP")
for i in "${WORKER_IDX[@]}"; do
  SERVERPOD_PUBLIC_IPS+=("${IPS[$i]}")
done
SERVERPOD_PUBLIC_IPS_TEXT="${SERVERPOD_PUBLIC_IPS[*]}"
SERVERPOD_PRIMARY_PUBLIC_IP="$MASTER_IP"
...
PUBLIC_IPS="${SERVERPOD_PUBLIC_IPS_TEXT}"
PRIMARY_PUBLIC_IP="${SERVERPOD_PRIMARY_PUBLIC_IP}"
...
echo "🔐 Gerando certificado TLS autoassinado para: \$PUBLIC_IPS"
{
  cat <<EOF
[req]
...
# Cada rota usa seu próprio Middleware/Ingress para evitar aplicar
# o stripPrefix de uma rota sobre a outra.
echo "📝 Gerando Middlewares e Ingress HTTPS..."
...
kind: Ingress
metadata:
  name: fibo-serverpod-api-ingress
  namespace: default
  annotations:
...
kind: Ingress
metadata:
  name: fibo-serverpod-control-ingress
  namespace: default
  annotations:
    traefik.ingress.kubernetes.io/router.entrypoints: websecure
    traefik.ingress.kubernetes.io/router.middlewares: default-control-strip-prefix@kubernetescrd
    traefik.ingress.kubernetes.io/router.tls: "true"
spec:
  ingressClassName: public
  tls:
...
echo "☸️  Validando webhook do CloudNativePG..."
cnpg_ready=0
for i in \$(seq 1 60); do
  ...
  if [[ -n "\$webhook_addr" ]]; then
    echo "    ✅ CNPG controller e webhook prontos: \$webhook_addr"
    cnpg_ready=1
    break
  fi

  if [[ \$((i % 10)) -eq 0 ]]; then
    echo "    ⏳ Aguardando webhook CNPG... \$((i * 3))s"
  fi
  sleep 3
done

if [[ \$cnpg_ready -ne 1 ]]; then
  echo "    ❌ Timeout: webhook do CNPG nao ficou pronto" >&2
  microk8s kubectl get pods -n cnpg-system -o wide >&2 || true
  exit 1
fi
...
echo
echo "=== Validando publicação HTTPS declarada pelo template Nix ==="

echo "=== Testando os endpoints públicos ==="

PUBLIC_FAILURES=0
for public_ip in "${SERVERPOD_PUBLIC_IPS[@]}"; do
  public_http_code=$(curl -ksS \
    --connect-timeout 5 \
    --max-time 20 \
    -o "/tmp/serverpod-public-response-${public_ip}" \
    -w '%{http_code}' \
    "https://${public_ip}/serverpod/" 2>/dev/null || true)

  if [[ "$public_http_code" =~ ^[1-5][0-9][0-9]$ ]] && [[ "$public_http_code" != "000" ]]; then
    echo "✅ https://${public_ip}/serverpod/ respondeu HTTP ${public_http_code}"
  else
    echo "⚠️  https://${public_ip}/serverpod/ não respondeu." >&2
    echo "    Confirme AWS TCP/443 e o device Incus serverpod-https no primeiro container desse host." >&2
...
echo "URLs Serverpod:"
for public_ip in "${SERVERPOD_PUBLIC_IPS[@]}"; do
  echo "  API:     https://${public_ip}/serverpod/"
  echo "  Control: https://${public_ip}/control/"
done
echo
echo "Observacao:"
echo "  O certificado é autoassinado e contém SAN para todos os IPs acima."
echo "========================================================"

if [[ $PUBLIC_FAILURES -gt 0 ]]; then
  echo "Concluido com ${PUBLIC_FAILURES} endpoint(s) externo(s) sem resposta." >&2
else
  echo "Concluido."
fi
