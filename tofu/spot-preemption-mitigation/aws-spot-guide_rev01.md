# AWS Amazon EC2 Spot — Implementation Guide (rev01)

> **rev01 — Correções aplicadas com base em revisão do Amazon Q:**
> 1. Softened legacy endpoint language (`termination-time` não é "deprecated", apenas `instance-action` é preferido)
> 2. Corrigido rebalance recommendation timing (pode chegar "antes ou ao mesmo tempo" que o aviso de 2 min, nem sempre antes)
> 3. Adicionados links de documentação supplementary identificados na revisão
...
> - [Initiate a Spot Instance interruption (testing)](https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/initiate-a-spot-instance-interruption.html)
...
> - [Spot Instance Best Practices (Whitepaper)](https://docs.aws.amazon.com/whitepapers/latest/cost-optimization-leveraging-ec2-spot-instances/spot-best-practices.html)
> - [Managing Spot Instance Interruptions (Whitepaper)](https://docs.aws.amazon.com/whitepapers/latest/cost-optimization-leveraging-ec2-spot-instances/managing-instance-termination.html)
...
> **Legacy endpoint:** `spot/termination-time` returns a plain-text timestamp. AWS documentation does not explicitly deprecate it, but recommends using `instance-action` instead, as it provides both the action type and the scheduled time in a structured JSON response.
...
AWS emits a **rebalance recommendation** signal when the instance is at **elevated risk** of interruption. Available via IMDSv2:
...
> **Timing note:** The rebalance recommendation can arrive **before or at the same time** as the 2-minute interruption notice — it is not guaranteed to always precede it. Treat it as an early heads-up when available, but do not rely on it as a prerequisite for the 2-minute warning.

When a rebalance recommendation is received, you can proactively start replacing the instance (e.g., launch a new Spot request and drain workloads) before the hard 2-minute deadline hits.
...
> **Tip:** AWS also provides a testing mechanism — you can [initiate a Spot Instance interruption](https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/initiate-a-spot-instance-interruption.html) on demand to test your handler against the real IMDSv2 endpoint without waiting for an actual capacity reclaim.
...
9. **Set rebalance recommendation alerts** — gives you a proactive heads-up, though it may arrive at the same time as the 2-minute notice rather than always before it.
10. **Test with On-Demand first** — terminate an On-Demand instance manually or use the [initiate interruption](https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/initiate-a-spot-instance-interruption.html) API to validate your graceful shutdown logic.
